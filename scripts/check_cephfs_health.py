...
def check_ceph_status():
    ...
    try:
        # Try ceph health
        result = subprocess.run(
            ["ceph", "health"], capture_output=True, text=True, timeout=5
        )
        ...
        # Try ceph mds stat
        result = subprocess.run(
            ["ceph", "mds", "stat"], capture_output=True, text=True, timeout=5
        )
        if result.returncode == 0:
        ...
        result = subprocess.run(
            ["ceph", "osd", "tree"], capture_output=True, text=True, timeout=5
        )
        if result.returncode == 0:
            # Parse for down OSDs
            down_osds = []
    ...
    if osd_issues or mds_issues:
        ...
        if suggestions:
            for sug in suggestions:
                ...
                if sug["monitor_ips"]:
                    print(
                        f"   Monitor IPs (in mount string): {', '.join(sug['monitor_ips'])}"
                    )
                if sug["osd_ips"]:
                    print(
                        f"   Problematic OSD IPs (from dmesg): {', '.join(sug['osd_ips'])}"
                    )
                if sug["overlap"]:
                    print(f"   ⚠️  OVERLAP DETECTED: {', '.join(sug['overlap'])}")
                    print(
                        "      These IPs serve BOTH as monitors AND have failing OSDs"
                    )
                    print("      This suggests a storage node issue on these hosts")
                else:
                    print(
                        "   ℹ️  Monitor IPs and OSD IPs are on different hosts (normal)"
                    )
                print(f"\n   {sug['note']}")
        ...
        print(
            "   1. ⚠️  Do NOT modify your mount string monitors - it won't help with OSD issues"
        )
