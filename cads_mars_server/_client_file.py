...
from .client import ClientError, ConnectionWithKeepAlive, Result
...
class RemoteMarsClientSession:
    ...
    def execute(self):
        ...
        try:
            ...
            r = requests.post(
                self.url,
                json=dict(request=self.request, environ=self.environ, type="file"),
                stream=False,
            )
...
class RemoteMarsClientCluster:
    ...
    def execute(self, request, environ):
        ...
        for r in request:
            ...
            result = self._execute(req, environ)
...
class MarsAdaptor:
    def __init__(self, request, env) -> None:
        self.request = request
        self.env = env
